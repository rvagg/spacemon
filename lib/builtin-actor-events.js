...
async function sectorActivatedOrUpdated (event, lotusApi, decodeMsgCb) {
  ...
  const [method, manifests] = decodedMsg
  ...
  const sectors = manifests.reduce((acc, manifest, i) => {
    ...
    const pieces = manifest.pieces.map((p) => {
      ...
      if (!f05 && p.notify.length > 0) {
        ...
        console.error(`Unexpected non-f05 data in sector ${manifest.sector} for ${event.msgCid.toString()}, notify: ${new TextDecoder().decode(jsonEncode(p.notify))}`)
      }
      return {
        cid: p.cid,
        size: p.size,
    ...
    acc.push({ sector: manifest.sector, pieces })
    return acc
  }, [])
  return { method, sectors }
}
