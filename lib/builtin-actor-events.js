...
const stateCollectors = {
  'sector-activated': async (event, lotusApi) => {
    ...
    if (batchReturn == null) {
      console.error(`Message ${event.msgCid} for sector-activated doesn't have a valid BatchReturn: ${new TextDecoder().decode(jsonEncode(returnEncoded))}`)
      return
    }

    // Attach our own piece manifest to the event: { sector: [ { cid, size, verified, sparkling } ] }
    return params.sectorActivations.reduce((acc, sa, i) => {
      if (batchReturn.failCodes.length > 0) {
        for (const fail of batchReturn.failCodes) {
          if (fail.index === i) {
            // this sector activation failed, don't include it
            console.error(`Sector activation failed: ${sa.sectorNumber} (${fail.code}) in message ${event.msgCid.toString()}`)
            return acc
          }
        }
      }
      acc[sa.sectorNumber] = sa.pieces.map((p) => {
        return {
          cid: p.cid,
          size: p.size,
          verified: p.verifiedAllocationKey != null,
          sparkling: p.notify.length === 0 // TODO: check whether f05 is in here
        }
      })
      return acc
    }, {})
