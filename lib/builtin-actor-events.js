...
// and their properties: { sector: [ { cid, size, verified, sparkling } ] 
async function sectorActivatedOrUpdated (event, lotusApi, decodeMsgCb) {
  ...
  const manifests = decodeMsgCb(msg)

  // console.log('manifests', new TextDecoder().decode(jsonEncode(manifests)))

  // Execution return
  const msgLookup = await lotusApi.stateSearchMsg(tipset, event.msgCid, 2, false)
  ...
  const returnEncoded = cborDecode(base64pad.baseDecode(msgLookup.Receipt.Return))
  ...
  // Attach our own piece manifest to the event: { sector: [ { cid, size, verified, sparkling } ] }
  return manifests.reduce((acc, manifest, i) => {
    ...
    acc[manifest.sector] = manifest.pieces.map((p) => {
      // notifies f05? address will be a byte array with [0, 5]
      const sparkling = !p.notify.some((n) => n.address.length === 2 && n.address[0] === 0 && n.address[1] === 5)
      if (sparkling && p.notify.length > 0) {
        // as of nv22 this shouldn't be allowed and should have failed
        console.log(`Unexpected sparkling data in sector ${manifest.sector} for ${event.msgCid.toString()}, notify: ${new TextDecoder().decode(jsonEncode(p.notify))}`)
      }
      return {
        cid: p.cid,
        size: p.size,
...
const stateCollectors = {
  'sector-activated': async (event, lotusApi) => {
    return sectorActivatedOrUpdated(event, lotusApi, function (msg) {
      ...
      // Params
      const paramsEncoded = cborDecode(base64pad.baseDecode(msg.Params))
      ...
      return params.sectorActivations
    })
  },
