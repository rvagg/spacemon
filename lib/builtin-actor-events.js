...
const stateCollectors = {
  'sector-activated': async (event, lotusApi) => {
    ...
    console.log('stateSearchMsg:', msgLookup)
    ...
    const rtrn = cborDecode(base64pad.baseDecode(msgLookup.Receipt.Return))
    const validator = miscValidators.BatchReturn
    if (!validator) {
      throw new Error('No schema for BatchReturn')
    }
    const batchReturn = validator.toTyped(rtrn)
    console.log('BatchReturn:', batchReturn, 'Return:', rtrn)
  }
}
