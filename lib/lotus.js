  ...
  */
  ...
  async stateSearchMsg (tipsetKeyCids, msgCid, limit, allowReplaced) {
    ...
    if (tipsetKeyCids.length > 0) {
      ...
      cacheFile = pathJoin(this.config.apiCachePath, `stateSearchMsg-${msgCid.toString()}-${limit}-${allowReplaced}-${tskCid.toString()}.json`)
      try {
  ...
  async chainGetMessage (msgCid) {
    ...
    const cacheFile = pathJoin(this.config.apiCachePath, `chainGetMessage-${msgCid.toString()}.json`)
    try {
      await fs.access(cacheFile)
      const cacheData = await fs.readFile(cacheFile)
      return jsonDecode(cacheData)
  ...
  async stateSectorGetInfo (address, sectorId, tipsetKeyCids) {
    ...
    if (tipsetKeyCids.length > 0) {
      const tskCid = await new TipSetKey(...tipsetKeyCids).cid()
      cacheFile = pathJoin(this.config.apiCachePath, `stateSectorGetInfo-${address}-${sectorId}-${tskCid.toString()}.json`)
      try {
        await fs.access(cacheFile)
        const cacheData = await fs.readFile(cacheFile)
        return jsonDecode(cacheData)
...
export async function createApi (config) {
  await fs.mkdir(config.apiCachePath, { recursive: true })
  return new LotusApi(config)
}
