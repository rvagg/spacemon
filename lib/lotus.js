...
export class LotusApi {
  ...
  constructor (lotusHttpRpc, lotusBackupHttpRpc, maxFilters, apiCachePath) {
    this.lotusHttpRpc = lotusHttpRpc
    this.lotusBackupHttpRpc = lotusBackupHttpRpc
  ...
  */
  ...
  async stateSearchMsg (tipsetKeyCids, msgCid, limit, allowReplaced) {
    ...
    if (tipsetKeyCids.length > 0) {
      tskCid = await new TipSetKey(...tipsetKeyCids).cid()
    ...
    // console.log('Filecoin.StateSearchMsg', JSON.stringify(params))
    let result = null
    try {
      result = await this.#request('Filecoin.StateSearchMsg', params)
    } catch (e) {
      console.error(`Error on StateSearchMsg for ${msgCid} for TipSetKey CID ${tskCid ? tskCid : '[]'} (will retry with backup): ${e}`)
    }
    // TODO: it's unclear why this is needed but it seems to help with occasional RPC errors,
    // perhaps a retry would be good enough?
    if (result == null) {
      const rpc = this.lotusHttpRpc
      this.lotusHttpRpc = this.lotusBackupHttpRpc
      try {
        console.log('Retrying StateSearchMsg with backup API without TipSet constraint...')
        params[0] = []
        result = await this.#request('Filecoin.StateSearchMsg', params)
      } finally {
        this.lotusHttpRpc = rpc
      }
    }
    if (tipsetKeyCids.length > 0 && result != null) {
  ...
  async chainGetMessage (msgCid) {
    ...
    let result = null
    try {
      result = await this.#request('Filecoin.ChainGetMessage', [msgCid])
    } catch (e) {
      console.error(`Error on ChainGetMessage for ${msgCid} (will retry with backup): ${e}`)
    }
    if (result == null) {
      const rpc = this.lotusHttpRpc
      this.lotusHttpRpc = this.lotusBackupHttpRpc
      try {
        console.log('Retrying ChainGetMessage with backup API...')
        result = await this.#request('Filecoin.ChainGetMessage', [msgCid])
      } finally {
        this.lotusHttpRpc = rpc
      }
...
export async function createApi (config) {
  ...
  return new LotusApi(config.lotusHttpRpc, config.lotusBackupHttpRpc, config.maxFilters, apiCachePath)
}
