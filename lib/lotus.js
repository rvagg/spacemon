...
// TODO: this isn't appropriate to be hard-wired, default lotus API has this at
// 10_000 so it should match whatever the node this is running against
const maxActorEventsResults = 100_000
  ...
  */
  ...
  async stateSearchMsg (tipsetKeyCids, msgCid, limit, allowReplaced) {
    ...
    try {
      ...
      console.error(`Error (will retry with backup): ${e}`)
    }
    /* This logic helped catch up on old messages that I didn't have locally, perhaps this would
       be useful in the future. It was only needed for StateSearchMsg, which I don't understand,
       perhaps something about receipts for a node with DisableHistoricalEvents enabled?
    */
    if (result == null) {
      const rpc = this.lotusHttpRpc
      this.lotusHttpRpc = 'https://api.zondax.ch/fil/node/mainnet/rpc/v1'
      try {
        console.log('Retrying with backup API...')
        result = await this.#request('Filecoin.StateSearchMsg', params)
      } finally {
